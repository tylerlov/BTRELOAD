...
namespace UltimateSpawner.Editor.Drawer
{
    ...
    internal sealed class SpawnableMaskDrawer : ImPropertyDrawer<SpawnableMask>
    {
        ...
        {
            //EditorGUI.BeginProperty(area, content, Property);
            ...
            {
                // Render a mask field
                bool result = GUIMaskField(next, "All (No Masked Items)", InspectedValue.NoMask);

                if (result != InspectedValue.NoMask)
                {
                    InspectedValue.NoMask = result;
                    InspectedValue.changed = true;
                }
                ...
                if (InspectedValue.NoMask == false)
                {
                    ...
                    for(int i = 0; i < items.Length; i++)
                    {
                        ...
                        if(isChecked != items[i].spawnableMasked)
                        {
                            ...
                            InspectedValue.changed = true;
                            //SerializedProperty noMask = Property.FindPropertyRelative("noMask");
                            //SerializedProperty maskedItems = Property.FindPropertyRelative("maskedItems");

                            //noMask.boolValue = InspectedValue.NoMask;


                            //while (maskedItems.arraySize < InspectedValue.GetMask().Count)
                            //    maskedItems.InsertArrayElementAtIndex(0);

                            //for (int j = 0; j < InspectedValue.GetMask().Count; j++)
                            //    maskedItems.GetArrayElementAtIndex(j).intValue = InspectedValue.GetMask()[j];


                            //SerializedProperty p = Property.FindPropertyRelative("changed");
