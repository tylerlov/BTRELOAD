...
namespace Koenigz.PerfectCulling
{
    ...
    public class PerfectCullingBakeDataEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            if (!string.IsNullOrEmpty(data.strRendererName))
            {
                using (new EditorGUILayout.HorizontalScope())
                {
                    EditorGUILayout.PrefixLabel("Renderer");
                    EditorGUILayout.LabelField(data.strRendererName);
                }
            }
            ...
            if (data.bakeDurationMilliseconds > 0)
            {
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    EditorGUILayout.LabelField(PerfectCullingEditorUtil.FormatSeconds(data.bakeDurationMilliseconds * 0.001f));
                }
            }
