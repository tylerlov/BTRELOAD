    ...
    public class PerfectCullingCameraEditor : Editor
    {
        ...
        void Setup(PerfectCullingCamera camera)
        {
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                if (includeNeighborCells.intValue != 0)
                {
                    EditorGUILayout.HelpBox("It is recommended to only use this radius option to quickly test whether additional Merge-Downsample iterations (located on PerfectCullingVolume; change requires re-bake) would be helpful to reduce culling artifacts.\n", MessageType.Warning);
                }
