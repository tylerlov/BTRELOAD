...
namespace Koenigz.PerfectCulling
{
    public class PerfectCullingSceneColor : IDisposable
    {
        ...
        public PerfectCullingSceneColor(PerfectCullingBakeGroup[] groups, HashSet<Renderer> additionalOccluders, bool setupRenderers = true)
        {
            ...
            {
                PerfectCullingLogger.LogError("Missing material");
            ...
            for (int groupIndex = 0; groupIndex < m_allGroups.Length; ++groupIndex)
            {
                foreach (Renderer renderer in m_allGroups[groupIndex].renderers)
                {
                    if (renderer == null)
                    {
                        PerfectCullingLogger.LogError("Some renderers are null. References to renderers have to be stable to not cause issues.");
                    ...
                    // Needs to be removed because MeshRenderer depends on it (if it exists)
                    if (go.TryGetComponent<PerfectCullingRendererTag>(out PerfectCullingRendererTag rt))
                    {
                        m_disposeList.Add(rt);
                    }
        ...
        void PrepareRenderer(Renderer renderer, Color col, MaterialPropertyBlock propBlock)
        {
            ...
            {
                bool isTransparent = PerfectCullingEditorUtil.IsMaterialTransparent(allMaterials[materialIndex]);
                ...
                col.a = (isTransparent ? PerfectCullingEditorUtil.TRANSPARENT_RENDER_COLOR : PerfectCullingEditorUtil.OPAQUE_RENDER_COLOR);
