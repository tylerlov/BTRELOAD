...
// Main Light Data
void WCE_getMainLitData(float3 WorldPos, half4 ShadowMask, out half3 litDir, out half3 litColor, out half litAtten,
                        out half litShadow)
{
...
void WCE_getAdditionalLitData(uint idx, float3 WorldPos, half4 ShadowMask, out half3 litDir, out half3 litColor,
                              out half litAtten, out half litShadow)
{
    #if defined(SHADERGRAPH_PREVIEW)
...
half WCE_axisYAttenAddLit(uint idx, float3 WorldPos, float WaterSurfaceY, float AxisYAttenBase,
                          float AxisYAttenWideSafe)
{
...
// 減衰をまとめる
half WCE_atten(half litAtten, half litShadow, half3 litDir, half dotLN, half axisYAtten, half NormalAttenIntensity,
               half NormalAttenPowSafe, half TransparentBack)
{
...
// テクスチャサンプリング
half3 WCE_sampleCausticsTex(float3 WorldPos, WCE_TEX_ARGS(CausticsTex), half3 litDir, float ScaleSafe,
                            float2 ColorShift, float WaterSurfaceY)
{
...
half3 WCE_waterCausticsEmission(float3 WorldPos, half3 NormalWS, WCE_TEX_ARGS(CausticsTex), float Scale,
                                float WaterSurfaceY, float AxisYAttenBase, float AxisYAttenWide, half IntensityMainLit,
                                half IntensityAddLit,
                                float2 ColorShift, half LitSaturation, half NormalAttenIntensity, half NormalAttenPow,
                                half TransparentBack)
{
    ...
    [branch] if (IntensityMainLit > 0)
    {
        ...
        [branch] if (litAtten * litShadow > 0)
        {
            ...
            half atten = WCE_atten(litAtten, litShadow, litDir, dotLN, axisYAtten, NormalAttenIntensity,
                                   NormalAttenPowSafe, TransparentBack);
            [branch] if (atten > 0)
            {
                half3 texColor = WCE_sampleCausticsTex(WorldPos, WCE_TEX_PARAMS(CausticsTex), litDir, ScaleSafe,
                                                       ColorShift, WaterSurfaceY);
