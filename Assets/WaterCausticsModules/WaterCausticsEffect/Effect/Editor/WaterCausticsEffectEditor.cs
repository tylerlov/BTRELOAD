...
namespace MH.WaterCausticsModules
{
    ...
    public class WaterCausticsEffectEditor : Editor
    {
        ...
        {
            findProps();
            _cam = (Camera.main != null && Camera.main.isActiveAndEnabled)
                ? Camera.main
                : GameObject.FindObjectOfType<Camera>(false);
        }
        ...
        private void openPipelineAssetButton()
        {
            ...
            using (new ColorScope(colorPinkContent))
                if (GUILayout.Button("Open Current Pipeline Asset Settings", EditorStyles.miniButtonLeft,
                        GUILayout.Width(250)))
                {
        ...
        private bool drawProperties()
        {
            ...
            // Shape Group
            drawRect();
            m_clipOutsideVolume.isExpanded =
                !EditorGUILayout.Foldout(!m_clipOutsideVolume.isExpanded, "Influence Scope");
                ...
                using (var check = new EditorGUI.ChangeCheckScope())
                {
                    ...
                    using (new IndentScope(5, 0))
                    {
                        ...
                        EditorGUILayout.PropertyField(m_layerMask,
                            new GUIContent("Layer Mask", "Specify the layers to draw the effect."));
                        EditorGUILayout.Space(1);
                        EditorGUILayout.PropertyField(m_clipOutsideVolume,
                            new GUIContent("Clip Outside Volume", "Not draw the outside of the volume."));
                        EditorGUILayout.Space(0.5f);
                        EditorGUILayout.PropertyField(m_useImageMask,
                            new GUIContent("Image Mask", "Masking with an image."));
                        if (m_useImageMask.boolValue)
                        {
                            using (new IndentScope(-2f, 0f))
                                EditorGUILayout.PropertyField(m_imageMaskTexture,
                                    new GUIContent("Texture", "Texture to use for masking."));
                        }

                        EditorGUILayout.Space(1);
                        m_cullMode.enumValueIndex = EditorGUILayout.Popup(
                            new GUIContent("Render Face", "Which face to draw."), m_cullMode.enumValueIndex,
                            _cullingEnumStr);
                    }
            ...
            if (!m_texture.isExpanded)
            {
                using (new IndentScope())
                {
                    using (var check = new EditorGUI.ChangeCheckScope())
                    {
                        var discTex = new GUIContent("Caustics Texture",
                            "Set the Render Texture specified as the output destination in the Texture Generator.");
                        if (useTextureWarning)
                        {
                            ...
                            using (new ColorScope(colorPinkContent))
                            {
                                ...
                                EditorGUILayout.BeginHorizontal();
                                GUILayout.FlexibleSpace();
                                if (GUILayout.Button("Search from this Scene", EditorStyles.miniButtonLeft,
                                        GUILayout.Width(150)))
                                {
                        ...
                        EditorGUILayout.PropertyField(m_scale,
                            new GUIContent("Scale", "Texture size at the height of the water surface."));
                        EditorGUILayout.PropertyField(m_waterSurfaceY,
                            new GUIContent("Water Surface Y",
                                "Height of the water surface. Y-axis. The projected position of the light is calculated with respect to this plane."));
                        using (new LabelAndIndentScope("Near Surface Attenuation", 2f, -2f, 2f))
                        {
                            EditorGUILayout.PropertyField(m_waterSurfaceAttenWide,
                                new GUIContent("Width", "Attenuation width near the water surface."));
                            EditorGUILayout.PropertyField(m_waterSurfaceAttenOffset,
                                new GUIContent("Offset", "Adjustment of height where attenuation starts."));
                        }
                        ...
                        using (new IndentScope(-2f, 2f))
                        {
                            ...
                            {
                                EditorGUILayout.PropertyField(m_adjustMainLit,
                                    new GUIContent("Main Light", "Adjust the intensity of the main light."));
                            }

                            using (new DisableScope(m_useAdditionalLights.boolValue))
                            {
                                EditorGUILayout.PropertyField(m_adjustAddLit,
                                    new GUIContent("Additional Lights",
                                        "Adjust the intensity of the additional lights."));
                            }
                        }

                        EditorGUILayout.PropertyField(m_colorShiftU,
                            new GUIContent("Color Shift X", "Shifts the RGB channels. X-axis."));
                        EditorGUILayout.PropertyField(m_colorShiftV,
                            new GUIContent("Color Shift Z", "Shifts the RGB channels. Z-axis."));
                        litleSpace();
                        EditorGUILayout.PropertyField(m_litSaturation,
                            new GUIContent("Light Color Saturation", "Color intensity of the light."));
                        using (new LabelAndIndentScope("Normal Attenuation", 2f, -2f, 2f))
                        {
                            EditorGUILayout.PropertyField(m_normalAttenIntensity,
                                new GUIContent("Intensity",
                                    "Attenuation due to the angle between the normal and the light ray."));
                            using (new DisableScope(m_normalAttenIntensity.floatValue > 0f))
                            {
                                EditorGUILayout.PropertyField(m_normalAttenPower,
                                    new GUIContent("Power", "Power the attenuation value."));
                                EditorGUILayout.PropertyField(m_transparentBackside,
                                    new GUIContent("Transparent Backside",
                                        "The intensity of light transmitted to the back side"));
                            }
                        }

                        EditorGUILayout.PropertyField(m_multiplyOpaqueColor,
                            new GUIContent("Multiply Opaque Color",
                                "Multiply opaque texture color. * Opaque Texture is required. Turn it on in the pipeline asset settings or in the camera settings."));
                        if (m_multiplyOpaqueColor.boolValue)
                        {
                            ...
                            using (new IndentScope(-2f, 0f))
                            {
                                EditorGUILayout.PropertyField(m_multiplyOpaqueIntensity,
                                    new GUIContent("Intensity", "The intensity of multiplying opaque texture colors."));
                                if (!opaqueTexAvailable)
                                {
                                    EditorGUILayout.Space(1);
                                    EditorGUILayout.HelpBox(
                                        "Opaque Texture is required to use this setting.\nTurn it on in the pipeline asset settings or in the camera settings.",
                                        MessageType.Warning);
                        ...
                        EditorGUILayout.PropertyField(m_useMainLight,
                            new GUIContent("Use Main Light", "Calculate the main light."));
                        EditorGUILayout.PropertyField(m_useAdditionalLights,
                            new GUIContent("Use Additional Lights", "Calculate the additional lights."));
                        EditorGUILayout.PropertyField(m_receiveShadows,
                            new GUIContent("Receive Shadows", "Calculate the shadows."));
            ...
            if (m_zTestMode.isExpanded)
            {
                using (new IndentScope())
                {
                    using (var check = new EditorGUI.ChangeCheckScope())
                    {
                        ...
                        EditorGUILayout.PropertyField(m_syncWithShaderFunctions,
                            new GUIContent("Sync With Custom Functions",
                                "Transmits the settings to the WaterCausticsEmissionSync function embedded in the shader. Turning this On will copy the settings to a global shader variable. If there is more than one of this effect at the same time, only one should be On."));

                        drawRect2WithLabel("Material");
                        m_zWriteMode.isExpanded = EditorGUILayout.Foldout(m_zWriteMode.isExpanded,
                            (m_zWriteMode.isExpanded ? "Culling" : "Culling / Depth"));
                        if (m_zWriteMode.isExpanded)
                        {
                            using (new IndentScope(2f, 0f))
                            {
                                EditorGUILayout.PropertyField(m_cullMode,
                                    new GUIContent("Cull",
                            ...
                            {
                                EditorGUILayout.PropertyField(m_zWriteMode,
                                    new GUIContent("ZWrite", "Whether to write depth values to the depth buffer."));
                                EditorGUILayout.PropertyField(m_zTestMode,
                                    new GUIContent("ZTest", "Comparison method with already existing depth values."));
                                litleSpace();
                                EditorGUILayout.PropertyField(m_depthOffsetFactor,
                                    new GUIContent("Offset Factor", "Offset Factor"));
                                EditorGUILayout.PropertyField(m_depthOffsetUnits,
                                    new GUIContent("Offset Units", "Offset Units"));
                            }
                        }
                        ...
                        if (m_stencilRef.isExpanded)
                        {
                            using (new IndentScope())
                            {
                                EditorGUILayout.PropertyField(m_stencilRef,
                                    new GUIContent("Ref", "Stencil Reference Value"));
                                EditorGUILayout.PropertyField(m_stencilReadMask,
                                    new GUIContent("ReadMask", "Stencil Read Mask"));
                                EditorGUILayout.PropertyField(m_stencilWriteMask,
                                    new GUIContent("WriteMask", "Stencil Write Mask"));
                                EditorGUILayout.PropertyField(m_stencilComp,
                                    new GUIContent("Comp", "Stencil Compare Operation"));
                                EditorGUILayout.PropertyField(m_stencilPass,
                                    new GUIContent("Pass", "Stencil Pass Operation"));
                                EditorGUILayout.PropertyField(m_stencilFail,
                                    new GUIContent("Fail", "Stencil Fail Operation"));
                                EditorGUILayout.PropertyField(m_stencilZFail,
                                    new GUIContent("ZFail", "Stencil Z Fail Operation"));
                            }
                        }
                        ...
                        {
                            EditorGUILayout.Space(-2);
                            EditorGUILayout.LabelField("Do not touch these values if there is no problem.",
                                new GUIStyle("HelpBox"));
                            EditorGUILayout.Space(2);
                            s_maintenanceUnlock = !EditorGUILayout.Toggle(
                                new GUIContent("Lock", "Do not touch these values if there is no problem."),
                                !s_maintenanceUnlock);
                            using (new DisableScope(s_maintenanceUnlock))
                            {
                                using (new IndentScope(3, 0))
                                {
                                    EditorGUILayout.PropertyField(m_noTexture,
                                        new GUIContent("No Texture Image",
                                            "Image to be displayed when no texture is found."));
                                    EditorGUILayout.Space(3);
                                    EditorGUILayout.PropertyField(m_shader,
                                        new GUIContent("Shader", "Shader for rendering effects."));
                                }
                            }
                        }
