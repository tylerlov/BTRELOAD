...
void WCE_WaterCausticsEmission_float(float3 WorldPos, half3 NormalWS, UnityTexture2D CausticsTex,
                                     float Scale, float WaterSurfaceY, float WaterSurfaceAttenWide,
                                     float WaterSurfaceAttenOffset, half IntensityMainLit,
                                     half IntensityAddLit, float ColorShiftU, float ColorShiftV, half LitSaturation,
                                     half NormalAttenIntensity,
                                     half NormalAttenPower, half TransparentBack, out half3 EmissionColor)
{
    EmissionColor = WCE_waterCausticsEmission(WorldPos, NormalWS, CausticsTex.tex, CausticsTex.samplerstate,
                                              ...
                                              float2(ColorShiftU, ColorShiftV) * 0.01, LitSaturation,
                                              NormalAttenIntensity, NormalAttenPower, TransparentBack);
}
