...
half3 WCE_WaterCausticsEmission(float3 WorldPos, half3 NormalWS, Texture2D CausticsTex, SamplerState CausticsTexSS,
                                float Scale = 3, float WaterSurfaceY = 2, float WaterSurfaceAttenWide = 0.5,
                                float WaterSurfaceAttenOffset = 0,
                                half IntensityMainLit = 1, half IntensityAddLit = 1, float ColorShiftU = 0.4,
                                float ColorShiftV = -0.1, half LitSaturation = 0.2,
                                half NormalAttenIntensity = 1, half NormalAttenPower = 2, half TransparentBack = 0)
{
    return WCE_waterCausticsEmission(WorldPos, NormalWS, CausticsTex, CausticsTexSS,
                                     Scale, WaterSurfaceY, WaterSurfaceY + WaterSurfaceAttenOffset,
                                     WaterSurfaceAttenWide, IntensityMainLit, IntensityAddLit,
                                     float2(ColorShiftU, ColorShiftV) * 0.01, LitSaturation, NormalAttenIntensity,
                                     NormalAttenPower, TransparentBack);
}
