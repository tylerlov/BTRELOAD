    ...
    public class WaterCausticsTexGeneratorEditor : Editor
    {
        ...
        private bool drawProperties()
        {
            ...
            if (m_generateInEditMode.isExpanded && useTextureWarning)
                ...
                drawRect();
            m_generateInEditMode.isExpanded =
                !EditorGUILayout.Foldout(!m_generateInEditMode.isExpanded, "Generate Texture");
                ...
                using (new IndentScope())
                {
                    using (var check = new EditorGUI.ChangeCheckScope())
                    {
                        ...
                        else
                        {
                            ...
                            EditorGUILayout.PropertyField(m_renderTexture,
                                new GUIContent("Output Render Texture", "Output destination render texture."));
                        }

                        var desc2 = new GUIContent("Calculate Resolution",
                            "Resolution used for internal calculations.");
                        m_calcResolution.enumValueIndex =
                            EditorGUILayout.Popup(desc2, m_calcResolution.enumValueIndex, _resEnumStr);

                        EditorGUILayout.PropertyField(m_FillGapAmount,
                            new GUIContent("Fill Gap",
                                "If the edges of the output image are not drawn, increase the value. Make it as small as possible to reduce the load."));
                        ...
                        if (targets.Length == 1)
                        {
                            ...
                            {
                                if (GUILayout.Button("Close Preview", EditorStyles.miniButtonLeft,
                                        GUILayout.Width(150)))
                                {
                            ...
                            else
                            {
                                using (new ColorScope(colorGreenButton))
                                    if (GUILayout.Button("Open Preview", EditorStyles.miniButtonLeft,
                                            GUILayout.Width(150)))
                                    {
                        ...
                        EditorGUILayout.PropertyField(m_density,
                            new GUIContent("Density", "Adjust the overall density."));
                        EditorGUILayout.PropertyField(m_speed, new GUIContent("Speed", "Adjust the overall speed."));
                        EditorGUILayout.Space(3);
                        EditorGUILayout.PropertyField(m_waves,
                            new GUIContent("Waves",
                                $"Wave settings. Supports up to {WaterCausticsTexGenerator.WAVE_MAX_CNT}."));
                        if (m_waves.arraySize >= 5)
                        {
                            EditorGUILayout.LabelField(
                                $"Up to {WaterCausticsTexGenerator.WAVE_MAX_CNT} waves be supported.",
                        ...
                        m_lightCondensingStyle.enumValueIndex = EditorGUILayout.Popup(desc0,
                            m_lightCondensingStyle.enumValueIndex, _condensingEnumStr);
                        EditorGUILayout.PropertyField(m_refractionIndex,
                            new GUIContent("Refraction Index", "Index of refraction."));


                        m_lightDirectionType.isExpanded =
                            EditorGUILayout.Foldout(m_lightDirectionType.isExpanded, "Details");
                        if (m_lightDirectionType.isExpanded)
                        {
                            EditorGUILayout.PropertyField(m_useChromaticAberration,
                                new GUIContent("Chromatic Aberration",
                                    "Simulate more realistic chromatic aberrations. Amount of calculation increases. For mobile devices, consider using color shift sampling at the time of drawing instead."));
                                ...
                                using (new IndentScope(-2, 5))
                                {
                                    EditorGUILayout.PropertyField(m_chromaticAberration,
                                        new GUIContent("Intensity",
                                            "Shifts the refractive index in the RGB channels."));
                                }
                            }

                            var desc1 = new GUIContent("Light Direction", "Direction of the ray from the Light.");
                            m_lightDirectionType.enumValueIndex = EditorGUILayout.Popup(desc1,
                                m_lightDirectionType.enumValueIndex, _litDirEnumStr);
                            ...
                            using (new IndentScope(1, 5))
                            {
                                switch ((WaterCausticsTexGenerator.LitDirTypeEnum)m_lightDirectionType.enumValueIndex)
                                {
                                    case WaterCausticsTexGenerator.LitDirTypeEnum.Numeric:
                                        EditorGUILayout.PropertyField(m_lightDir,
                                            new GUIContent("Direction",
                                                "Direction of the rays from the Light. It will be normalized."));
                                        break;
                                    case WaterCausticsTexGenerator.LitDirTypeEnum.Transform:
                                        EditorGUILayout.PropertyField(m_lightTransform,
                                            new GUIContent("Transform", "Transforms the light to be referenced."));
                                        break;
                                    case WaterCausticsTexGenerator.LitDirTypeEnum.LitSettingSun:
                                        EditorGUILayout.LabelField("Use the sun setting in the Light Settings window.",
                                            new GUIStyle("HelpBox"));
                                        break;
                                    case WaterCausticsTexGenerator.LitDirTypeEnum.Auto:
                                    default:
                                        EditorGUILayout.LabelField(
                                            "Use the shader's global variable \"_LightDirection\".",
                                            new GUIStyle("HelpBox"));
                                        break;
                                }
                            }

                            EditorGUILayout.PropertyField(m_rayStyle,
                                new GUIContent("Ray Style",
                                    "Either normalize the rays past the surface of the water or extend them to the bottom. If the light ray is oblique, there is a noticeable difference."));
                        }
            ...
            if (m_computeShader.isExpanded)
            {
                using (new IndentScope())
                {
                    using (var check = new EditorGUI.ChangeCheckScope())
                    {
                        ...
                        EditorGUILayout.PropertyField(m_generateInEditMode,
                            new GUIContent("Generate In EditMode", "Whether to generate while in edit mode."));
                        ...
                        if (s_maintenanceOpen)
                        {
                            ...
                            using (new DisableScope(s_maintenanceUnlock))
                            {
                                ...
                                {
                                    EditorGUILayout.PropertyField(m_computeShader,
                                        new GUIContent("Compute Shader", "Compute Shader"));
