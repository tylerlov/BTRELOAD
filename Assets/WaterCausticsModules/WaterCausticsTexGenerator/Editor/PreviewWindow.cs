...
namespace MH.WaterCausticsModules
{
    internal class PreviewWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            if (isLockable)
                EditorGUI.DrawPreviewTexture(getIconRect(30f, 10f), isLocked ? m_iconLock : m_iconUnlock, matIcon(),
                    ScaleMode.StretchToFill, 0);
            if (isPauseable)
                EditorGUI.DrawPreviewTexture(getIconRect(50f, 10f), s_isPause ? m_iconPause : m_iconResume, matIcon(),
                    ScaleMode.StretchToFill, 0);
        ...
        private void openContext()
        {
            ...
            if (isPauseable)
            {
                menu.AddItem(new GUIContent("Pause", "Stop the movement of the waves."), s_isPause,
                    (o) => togglePause(), null);
                menu.AddSeparator("");
            }
            ...
            if (isLockable)
            {
                menu.AddItem(new GUIContent("Lock", "Unlocked when entered into play mode."), isLocked,
                    (o) => toggleLock(), null);
                menu.AddSeparator("");
            }
            ...
            if (_rt && _rt.width == _rt.height)
            {
                ...
                menu.AddItem(new GUIContent($"{curSize}Fit Texture {rtW}x{rtW}"), winW == _rt.width, specifyWindowSize,
                    (float)_rt.width);
            ...
            {
                menu.AddSeparator("");
                menu.AddItem(new GUIContent("Select Generator :  " + _target.name), false,
                    (o) => pingAndSelect(_target, true), null);
                if (_target.renderTexture != null)
                {
                    bool isLocking = isLockable && isLocked;
                    string action = isLocking ? "Select" : "Ping";
                    menu.AddItem(new GUIContent(action + " Texture :  " + _target.renderTexture.name), false,
                        (o) => pingAndSelect(_target.renderTexture, isLocking), null);
                    if (_rt != null)
                    {
                        menu.AddSeparator("");
        ...
        private void savePng(object o)
        {
            ...
            string defaultPath = string.IsNullOrEmpty(_preSavePngPath)
                ? Application.dataPath
                : Path.GetDirectoryName(_preSavePngPath);
            string defaultName = string.IsNullOrEmpty(_preSavePngPath)
                ? "WaterCausticsTexSnapshot"
                : Path.GetFileNameWithoutExtension(_preSavePngPath);
