...
namespace Highlighters
{
    ...
    public class Highlighter : MonoBehaviour
    {
        ...
        // Add these fields near the top of the Highlighter class
        private bool isActivating = false;
        private float activationProgress = 0f;
        private float activationDuration = 0.5f; // Adjust this value to control how long the fade-in takes
        private float initialBlurIntensity;

        // Modify OnEnable() to start a gradual activation
        public void OnEnable()
        {
            Setup();
            
            // Store the initial blur intensity and start at 0
            initialBlurIntensity = Settings.BlurIntensity;
            Settings.BlurIntensity = 0f;
            
            // Start gradual activation
            isActivating = true;
            activationProgress = 0f;
            StartCoroutine(GradualActivation());
            
            HighlighterValidate();
        }

        // Add this new coroutine
        private IEnumerator GradualActivation()
        {
            while (activationProgress < 1f)
            {
                activationProgress += Time.deltaTime / activationDuration;
                Settings.BlurIntensity = Mathf.Lerp(0f, initialBlurIntensity, activationProgress);
                
                // Update the highlighter every frame during activation
                HighlighterValidate();
                
                yield return null;
            }
            
            // Ensure we end up at exactly the target value
            Settings.BlurIntensity = initialBlurIntensity;
            isActivating = false;
            HighlighterValidate();
        }

        // Modify OnDisable() to handle cleanup
        public void OnDisable()
        {
            if (isActivating)
            {
                StopCoroutine(GradualActivation());
            }
