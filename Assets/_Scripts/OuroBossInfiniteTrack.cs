...
namespace FluffyUnderware.Curvy.Examples
{
    public class OuroBossInfiniteTrack : MonoBehaviour
    {
        ...
        public bool isGenerated = false; // Flag to check if generation is done
        ...
        // Call this method to clear the generated items
        public void Clear()
        {
            // Your clearing logic here
            // Example: Clearing generated control points, prefabs, etc.
            ...
            foreach (var section in prefabsBySection)
            {
        ...
        IEnumerator setup()
        {
            ...
            if (TrackSpline.ControlPointCount > 0)
            {
                // Check if the first control point is at (0, 0, 0) and remove it
                var firstCP = TrackSpline.ControlPointsList[0];
                if (firstCP.transform.position == Vector3.zero)
                {
                    TrackSpline.Delete(firstCP);
                }
            }
        }
...
#pragma warning restore 618
        ...
        void addTrackCP()
        {
            if (ReferenceSpline == null || ReferenceSpline.ControlPointCount == 0)
            {
                Debug.LogError("Reference spline is not set or has no control points.");
                return;
            }

            // Calculate the total number of control points needed for a closed loop
            int totalCPNeeded = Sections * SectionCPCount + TailCP + HeadCP;
            // Check if adding another control point would exceed this number
            if (TrackSpline.ControlPointCount >= totalCPNeeded)
            {
                // Stop adding more control points to avoid unnecessary overlap
                return;
            }

            // Correctly initialize the first control point based on your logic
            if (TrackSpline.ControlPointCount == 0)
            {
                // Calculate the position for the first control point based on your logic
                // For example, using the ReferenceSpline to determine the starting position
                float totalLength = ReferenceSpline.Length;
                float distancePerCP = totalLength / (Sections * SectionCPCount);
                // Adjusting for the first control point to not start at 0 distance
                float nextCPDistance = distancePerCP; // Start at the distance for the first CP instead of 0
            ...
            {
                // For subsequent control points, continue with the existing logic
                float totalLength = ReferenceSpline.Length;
                float distancePerCP = totalLength / (Sections * SectionCPCount);
                float nextCPDistance = (TrackSpline.ControlPointCount + TailCP - 1) * distancePerCP; // Adjusted to account for the initial control point

                // Ensure the distance wraps around the reference spline if it exceeds its total length
                nextCPDistance %= totalLength;
            ...
            // Ensure the spline is refreshed to update its state
            TrackSpline.Refresh();
        }
