...
{
    [SerializeField] [Tooltip("Number of full rotations per second")]
    private float rotationsPerSecond = 0.5f;
    [SerializeField] [Tooltip("Distance from the parent object's center")]
    private float radius = 0.5f;
    [SerializeField] private Vector3 circularMotionAxis = Vector3.up;
    [SerializeField] private float height = 0f;
    [SerializeField] [Range(0f, 1f)] [Tooltip("Higher values create smoother motion but less responsiveness")]
    private float positionSmoothness = 0.95f;
    [SerializeField] [Range(0f, 1f)] [Tooltip("Higher values create smoother rotation but less responsiveness")]
    ...
    private void Start()
    {
        ...
        {
            Debug.LogError("TailOrientation: Parent object must have CustomAIPathAlignedToSurface and Timeline components.");
            enabled = false;
            return;
        }
    ...
    private void Update()
    {
        if (parentAI == null || parentTimeline == null) return;
        ...
        currentPosition = Vector3.Lerp(currentPosition, targetPosition, 1f - Mathf.Pow(positionSmoothness, deltaTime * 60f));
        currentRotation = Quaternion.Slerp(currentRotation, targetRotation, 1f - Mathf.Pow(rotationSmoothness, deltaTime * 60f));
    ...
    private Vector3 CalculateTargetPosition(float deltaTime)
    {
        ...
        Vector3 projectedOffset = Vector3.ProjectOnPlane(offset, circularMotionAxis).normalized * radius;
    ...
    private Quaternion CalculateTargetRotation()
    {
        ...
        Vector3 orientationDirection = Vector3.ProjectOnPlane(parentToTail, parentAI.transform.up).normalized;
        
        float behindFactor = Vector3.Dot(parentVelocity.normalized, -parentToTail.normalized);
        behindFactor = Mathf.Clamp01(behindFactor);
        
        Vector3 blendedDirection = Vector3.Lerp(orientationDirection, parentVelocity.normalized, behindFactor);
    ...
    private void OnDrawGizmos()
    {
        ...
        Vector3 radiusPoint = circleCenter + Vector3.ProjectOnPlane(Vector3.right, circularMotionAxis).normalized * radius;
        ...
        Vector3 speedVisualizationPoint = circleCenter + Vector3.ProjectOnPlane(Vector3.right, circularMotionAxis).normalized * (radius + 0.2f);
        UnityEditor.Handles.color = Color.yellow;
        UnityEditor.Handles.DrawWireArc(circleCenter, circularMotionAxis, Vector3.ProjectOnPlane(Vector3.right, circularMotionAxis), 360 * rotationsPerSecond, radius + 0.2f);
    ...
    private void DrawCircle(Vector3 center, Vector3 axis, float radius, int segments)
    {
        ...
        for (int i = 1; i <= segments; i++)
        {
            ...
            Vector3 nextPoint = center + Quaternion.AngleAxis(angle * Mathf.Rad2Deg, up) * (right * radius);
