...
namespace FIMSpace.FTail
{
    ...
    public class FTailAnimator_Editor_Base : Editor
    {
        ...
        public string Title = "Tail Animator 2";
        ...
        protected virtual void ConvertToV2(FTail_AnimatorBase oldTail, TailAnimator2 n)
        {
            n.SmoothingStyle = TailAnimator2.EAnimationStyle.Quick;
            n.Slithery = 1.2f;

            n.ReactionSpeed = oldTail.PositionSpeed / 45f;
            n.RotationRelevancy = Mathf.Lerp(0.4f, 1f, Mathf.InverseLerp(0f, 45f, oldTail.RotationSpeed) );
            
            if ( oldTail.TailTransforms.Count > 0 )
            {
                n.StartBone = oldTail.TailTransforms[0];

                if (oldTail.TailTransforms.Count > 1)
                {
                    n.EndBone = oldTail.TailTransforms[oldTail.TailTransforms.Count - 1];
                }
            }

            n.UpdateRate = 0;

            n.Springiness = oldTail.Springiness;
            //n.Curling = oldTail.Sensitivity +
            //    Mathf.Lerp(0.0f, 0.45f, Mathf.InverseLerp(60f, 10f, oldTail.RotationSpeed) );
            //n.Curling = Mathf.Clamp(n.Curling, 0.1f, .825f);

            n.Curling = Mathf.Lerp(0.06f, 0.4f, Mathf.InverseLerp(0.2f, 0.9f, oldTail.Sensitivity));

            //n.Curling = Mathf.Clamp(n.Curling, 0.1f, .825f);

            n.MaxStretching = oldTail.MaxStretching;
            n.AngleLimit = oldTail.AngleLimit;
            if (oldTail.AngleLimit > 88) n.AngleLimit = 181;
            n.AngleLimitAxis = oldTail.AngleLimitAxis;
            n.LimitSmoothing = oldTail.LimitSmoothing;

            n.MotionInfluence = oldTail.MotionInfluence;
            n.UpdateAsLast = oldTail.QueueToLastUpdate;

            n.LengthMultiplier = oldTail.LengthMultiplier;

            //if (!oldTail.AnimateCorrections) n.SyncWithAnimator = false;

            if (!oldTail.SafeDeltaTime) n.DeltaType = TailAnimator2.EFDeltaType.DeltaTime;

            n.Curving = Quaternion.Euler(oldTail.Curving);
            n.Gravity = oldTail.Gravity;
            n.OptimizeWithMesh = oldTail.OptimizeWithMesh;
            //n.IncludeParent = oldTail.RootToParent;

            if ( oldTail.AngleLimit < 90)
            {
                n.AngleLimit = Mathf.Lerp(12, 110f, oldTail.AngleLimit / 90f);
            }

            if (oldTail.UseCollision)
            {
                n.UseCollision = oldTail.UseCollision;
                n.CollidersScaleCurve = oldTail.CollidersScale;
                
                if (oldTail.CollisionSpace == FTail_AnimatorBase.ECollisionSpace.Selective_Fast)
                    n.CollisionSpace = TailAnimator2.ECollisionSpace.Selective_Fast;
                else
                    n.CollisionSpace = TailAnimator2.ECollisionSpace.World_Slow;

                n.CollideWithOtherTails = oldTail.CollideWithOtherTails;
                n.CollidersAddRigidbody = oldTail.CollidersAddRigidbody;
                n.RigidbodyMass = oldTail.RigidbodyMass;
                n.CollidersLayer = oldTail.CollidersLayer;
                n.CollidersSameLayer = oldTail.CollidersSameLayer;
                n.IncludedColliders = oldTail.IncludedColliders;
                n.IgnoredColliders = oldTail.IgnoredColliders;
                n.CollidersScaleCurve = oldTail.CollidersScale;
                n.CollidersScaleMul = oldTail.CollidersScaleMul;
                n.CollisionsAutoCurve = oldTail.DifferenceScaleFactor;
                n.ReflectCollision = Mathf.Lerp(.25f, .8f, Mathf.InverseLerp(.85f, .25f, oldTail.CollisionSwapping));
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("CONVERT TO Tail Animator V2", GUILayout.Height(30)))
            {
                foreach (GameObject item in Selection.objects)
                {
                    FTail_AnimatorBase ta = item.GetComponent<FTail_AnimatorBase>();
                    TailAnimator2 t = ta.gameObject.AddComponent<TailAnimator2>();
                    ConvertToV2(ta, t);
                    t.GetGhostChain();
                }
            ...
            if (GUILayout.Button("CONVERT + Destroy", GUILayout.Height(30)))
            {
                foreach (GameObject item in Selection.objects)
                {
                    FTail_AnimatorBase ta = item.GetComponent<FTail_AnimatorBase>();
        ...
        protected virtual void DrawPhysicalParametersGUI(FTail_AnimatorBase tail)
        {
            ...
            if (tail.UseCollision) EditorGUILayout.BeginVertical(FGUI_Resources.BGInBoxStyle);
