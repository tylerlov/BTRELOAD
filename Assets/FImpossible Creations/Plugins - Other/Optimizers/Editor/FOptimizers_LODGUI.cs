...
{
    internal static class FOptimizers_LODGUI
    {
        #region Pre Defines
        ...
        /// </summary>
        public static void SetSelectedLODLevelPercentage(float newPercentage, int lod, List<FOptimizers_LODInfo> lods)
        {
            if (lod < 0) return;

            // Find the lower detail lod, clamp value to stop overlapping slider
            float minimum = 0.0f;
            FOptimizers_LODInfo lowerLOD = lods.FirstOrDefault(x => x.LODLevel == lods[lod].LODLevel - 1);
            if (lowerLOD != null) minimum = lowerLOD.LODPercentage;

            // Find the higher detail lod, clamp value to stop overlapping slider
            float maximum = 1.0f;
            FOptimizers_LODInfo higherLOD = lods.FirstOrDefault(x => x.LODLevel == lods[lod].LODLevel + 1);
        ...
        /// </summary>
        public static void DrawLODSlider(FOptimizer_Base optimizer, Rect area, IList<FOptimizers_LODInfo> lodInfos, int selectedLevel, float currentDistance = 0f, float maxDistance = 0f, bool hidden = false, int startCull = 0)
        {
            Rect areaA = area;
            ...
            {
                FOptimizers_LODInfo lodInfo = lodInfos[i];
            ...
            DrawCulledRange(areaA, lodInfos.Count > 0 ? prePerc : 1.0f, lodInfos[0].MinMax, selectedLevel == lodInfos.Count);
            ...
            if (hidden)
            {
                ...
                DrawHiddenRange(area, cullStartPercentage, lodInfos[0].MinMax, selectedLevel == lodInfos.Count + 1);
            }
        }
        ...
        public static Rect CalcLODButton(Rect totalRect, float percentage, bool hidden)
        {
            ...
            return new Rect(totalRect.x + startX - 5, totalRect.y, 10, totalRect.height - (hidden ? 14 : 0));
        }
        ...
        /// </summary>
        private static void DrawLODButton(FOptimizers_LODInfo currentLOD)
        {
        ...
        /// </summary>
        private static void DrawLODRange(FOptimizers_LODInfo currentLOD, float previousLODPercentage, bool isSelected, float nextUnits, int lodLevels)
        {
            ...
            float preUnits = 0f;
            if (preUnits == 0f) preUnits = Mathf.Lerp(currentLOD.MinMax.x, currentLOD.MinMax.y, previousLODPercentage);
        ...
        /// </summary>
        private static void DrawCulledRange(Rect totalRect, float previousLODPercentage, Vector2 minMax, bool isSelected = false)
        {
        ...
        private static void DrawHiddenRange(Rect totalRect, float cullStartPerc, Vector2 minMax, bool isSelected = false)
        {
        ...
        /// </summary>
        public sealed class FOptimizers_LODInfo
        {
            public Rect ButtonRect;
            public Rect RangeRect;

            public FOptimizers_LODInfo(int lodLevel, string name, float percentage)
            {
                LODLevel = lodLevel;
                LODName = name;
            ...
            public Vector2 MinMax { get; internal set; }
        }
