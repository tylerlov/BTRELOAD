...
namespace FIMSpace.FOptimizing
{
    ...
    /// </summary>
    [AddComponentMenu("FImpossible Creations/Optimizers/Terrain Optimizer")]
    public class FOptimizer_Terrain : FOptimizer_Base, UnityEngine.EventSystems.IDropHandler, IFHierarchyIcon
    {
        #region Hierarchy icons
        public string EditorIconPath { get { return "FIMSpace/FOptimizing/Optimizers Icon Terrain"; } }
        ...
        private void AddTerrainToOptimize()
        {
            ...
            if (ToOptimize.Count == 0)
            {
                ...
                {
                    FComponentLODsController controller = LoadLODReference("Optimizers/Base/FLOD_Terrain Reference").GenerateLODController(transform, this);
        ...
        protected override void OnDrawGizmosSelected()
        {
            ...
            if (isResizing == -1)
            {
                if (isSelected == LODLevels)
                {
                    ...
                    DrawTerrainSphere(MinMaxDistance.y, MinMaxDistance.y);
                }
                else
                    for (int i = 0; i < LODLevels; i++)
                    {
                        ...
                        float radius = Mathf.Lerp(MinMaxDistance.x, MinMaxDistance.y, LODPercent[i]);

                        float inRadius = radius;
                        if (i != 0) inRadius = radius - Mathf.Lerp(MinMaxDistance.x, MinMaxDistance.y, LODPercent[i - 1]);
                        else
                            inRadius = radius;
            ...
            else // when resizing sphere
            {
                ...
                float radius = Mathf.Lerp(MinMaxDistance.x, MinMaxDistance.y, LODPercent[isResizing]);
                ...
                if (isResizing != 0) inRadius = radius - Mathf.Lerp(MinMaxDistance.x, MinMaxDistance.y, LODPercent[isResizing - 1]);
                else
                    inRadius = radius;
            ...
            DrawTerrainSphere(MinMaxDistance.y + 1f);

            if (!Application.isPlaying)
            {
                Vector3 infoDir = new Vector3(1f, 1f, 0f).normalized;
                UnityEditor.Handles.Label(GetTerrainCenter() + transform.TransformDirection(infoDir * (GetMinRadius() + MinMaxDistance.y * LODPercent[0])), new GUIContent("[i]", "This frames in different colors indicates distance levels from terrain for Level Of Details (LODs)"));
            }
        }
