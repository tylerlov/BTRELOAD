...
float3 EstimateLightScattering( TEXTURE2D_X(OccluderTex), float2 UV, float FogDensity, bool SoftenScreenEdges, bool AnimateNoise, float MaxRayDistanceScreen, int NumberOfSamples, float3 SunColorTint, bool LightMustBeOnScreen, float FalloffIntensity)
{
	...
	float3 viewVector;
	[branch]
	if (_FalloffDirective == 0 && unity_OrthoParams.w == 0.0)
	{
